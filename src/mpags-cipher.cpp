...
int main(int argc, char* argv[])
{
    ...
    if (!CLA.inputFile.empty()) {
        // Open the file and check that we can read from it
        std::ifstream inputStream{CLA.inputFile};
        if (!inputStream.good()) {
            std::cerr << "[error] failed to create istream on file '"
                      << CLA.inputFile << "'" << std::endl;
            return 1;
        }
    ...
    if (!CLA.outputFile.empty()) {
        ...
        std::ofstream outputStream{CLA.outputFile};
        if (!outputStream.good()) {
            std::cerr << "[error] failed to create ostream on file '"
                      << CLA.outputFile << "'" << std::endl;
            return 1;
        }
